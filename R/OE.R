get.OE.sc <- function(r,gene.sign = NULL,num.rounds = 1000,mat.return.flag = T){
  ...
  r$genes.mean<-rowMeans(r$count)
  r$genes <-rownames(r$count)
  r$zscores<-sweep(r$count,1,r$genes.mean,FUN = '-')

  r$genes.dist<-rowMeans(r$count>0)
 
  r$genes.dist.q<-discretize(r$genes.dist,n.cat = 50)
  r$sign.scores<-matrix(data = 0,nrow = ncol(r$count),ncol = length(gene.sign))
  sign.names<-names(gene.sign)
  colnames(r$sign.scores)<-sign.names
  rownames(r$sign.scores)<-colnames(r$count)
